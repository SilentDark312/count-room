...
- **Play** — real blackjack with a play-money bankroll, a shoe size from 1 to 8 decks, resplitting up to 4 hands, late surrender, an optional basic-strategy hint on every hand (including surrender in the two cells where it's actually correct), a separate toggle for count-based index plays, a toggle to reveal the running/true count as you play, and light synthesized sound (chips, cards, win/lose) with a mute button in the header. Table rules follow the shoe size the way real casinos actually pair them: 1 deck plays dealer-hits-soft-17 with double-after-split allowed, 2 decks plays dealer-stands with DAS, and 3+ decks plays the standard no-DAS rules. Three modes: **Free Play**, **Challenge** (fixed $500 stake, play till you bust, best run tracked locally), and **Count Drill** (hides the count and index-play hint, and checks your tracked running count against the real one before every deal — the one mode that combines live counting with actually playing hands).
- **Trainer** — a consolidated progress dashboard up top (every stat below in one place, not scattered per pane), then three drills: a Strategy Trainer (one hand + dealer upcard at a time, pick the move, instant feedback), a speed drill that flashes cards for you to keep a running Hi-Lo count against, and a true-count quiz drilling the running-count ÷ decks-remaining arithmetic.
